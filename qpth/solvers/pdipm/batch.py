...
def factor_solve_kkt_reg(Q_tilde, D, G, A, rx, rs, rz, ry, eps):
    ...
    invH_A_ = lu_solve_batched(A_.transpose(1, 2), *H_LU)
    invH_g_ = lu_solve_batched(g_.unsqueeze(2), *H_LU).squeeze(2)
    ...
    w_ = -lu_solve_batched(t_.unsqueeze(2), *S_LU).squeeze(2)
    t_ = -g_ - w_.unsqueeze(1).bmm(A_).squeeze()
    v_ = lu_solve_batched(t_.unsqueeze(2), *H_LU).squeeze(2)
...
def solve_kkt(Q_LU, d, G, A, S_LU, rx, rs, rz, ry):
    ...
    invQ_rx = lu_solve_batched(rx.unsqueeze(2), *Q_LU).squeeze(2)
    if neq > 0:
    ...
    w = -(lu_solve_batched(h.unsqueeze(2), *S_LU)).squeeze(2)
    ...
    dx = lu_solve_batched(g1.unsqueeze(2), *Q_LU).squeeze(2)
    ds = g2 / d
    dz = w[:, neq:]
...
""")
    ...
    G_invQ_GT = torch.bmm(G, lu_solve_batched(G.transpose(1, 2), *Q_LU))
    R = G_invQ_GT.clone()
    S_LU_pivots = torch.IntTensor(range(1, 1 + neq + nineq)).unsqueeze(0) \
        .repeat(nBatch, 1).type_as(Q).int()
    if neq > 0:
        invQ_AT = lu_solve_batched(A.transpose(1, 2), *Q_LU)
        ...
        U_A_invQ_AT_inv = lu_solve_batched(P_A_invQ_AT.bmm(L_A_invQ_AT), *LU_A_invQ_AT)
        S_LU_21 = G_invQ_AT.bmm(U_A_invQ_AT_inv)
        T = lu_solve_batched(G_invQ_AT.transpose(1, 2), *LU_A_invQ_AT)
